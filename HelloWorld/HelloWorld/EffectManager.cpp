	...
	if (m_selectable.GetSelected())
	{
		...
		if (ImGui::TreeNode("Vignette Effect"))
		{
			if (m_vignette != -1)
			{
				...
				if (ImGui::DragFloat("Inner Radius", &innerRad, 0.01f, 0.f, 1.f))
				{
					temp->SetInnerRadius(innerRad);
				}
				if (ImGui::DragFloat("Outer Radius", &outerRad, 0.01f, 0.f, 1.f))
				{
		...
		if (ImGui::TreeNode("Pixelation Effect"))
		{
			if (m_pixelate != -1)
			{
				...
				float strength = temp->GetPixelSize();

				ImGui::Text("Currently Attached");

				if (ImGui::DragFloat("Pixel Size", &strength, 0.01f, 0.f, 50.f))
				{
					temp->SetPixelSize(strength);
				}
				if (ImGui::Button("Remove Effect", ImVec2(100.f, 20.f)))
				{
		...
		if (ImGui::TreeNode("Selective Colour Effect"))
		{
			if (m_selectiveColor != -1)
			{
				...
				if (ImGui::SliderFloat3("Colour Range 2", &colourRange2[0], 0.f, 1.f))
				{
					temp->SetColorRange1(vec3(colourRange2[0], colourRange2[1], colourRange2[2]));
				}
				if (ImGui::Button("Remove Effect", ImVec2(100.f, 20.f)))
				{
		...
		if (ImGui::TreeNode("Bloom Effect"))
		{
			if (m_bloom != -1)
			{
				...
				if (ImGui::DragFloat("Passes", &passes, 0.01f, 0.f, 10.f))
				{
					temp->SetPasses(passes);
				}
				if (ImGui::DragFloat("Threshold", &threshold, 0.01f, 0.f, 1.f))
				{
